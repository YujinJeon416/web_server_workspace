...
public class Member implements HttpSessionBindingListener {
	...
	//sql.date로 임포트->시분초정보가 필요할때는 timestamp 쓴다.
	...
	/**
	 * 세션객체에 현재객체가 속성으로 등록될때 호출되는 event listener : memberLogIn 등록되는 경우
	 */
	@Override
	public void valueBound(HttpSessionBindingEvent ev) {
		System.out.println("[" + memberId + "]님이 로그인 했습니다.");
	}
	/**
	 * 세션객체에 현재객체가 속성에서 해제될때 호출되는 event listener
	 * 세션이 무효화될때도 호출.
	 */

	@Override
	public void valueUnbound(HttpSessionBindingEvent ev) {
		System.out.println("[" + memberId + "]님이 로그아웃 했습니다.");		
	}
